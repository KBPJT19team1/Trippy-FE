...
const messages = ["타인으로부터 통장대여 요청을 받은 사실이 있나요?", "타인으로부터 신용점수 상향, 대출 등의 목적으로 통장개설을 요청받은 사실이 있나요?"];
...
const allChecked = computed(() => YesNoCheckeds.value && isPinMatched.value && accountNameChecked.value);
...
<template>
  ...
  <div class="w-full h-[8rem] flex flex-col items-center mt-5">
    ...
    <p v-if="passwordCheckedMessage" :class="[isPinMatched ? 'text-blue-600' : 'text-red']" class="caption3">
  ...
  <div class="w-full">
    ...
    <input type="text" placeholder="모임통장 이름" v-model="groupAccountName"
      class="w-full h-16 text-center rounded-xl mt-3 shadow-custom body1">
  </div>

  <AccountNotice class="mt-12 text-center " :title="'고객님의 자산을 안전하게 보호하고 전화 금융사기의 피해를 예방하고자 금융거래목적에 대해 질문드립니다.'" />

  <YesNoSelector v-for="(message, i) in messages" :key="i" :title="message" v-model="YesNoChecked[i]" />

  <NextButton :title="'다음'" :To="'group-account-step2'" :visible="allChecked" />
</template>
